...
from transformer import WEIGHTS_NAME
from transformer import (BertConfig, BertTokenizer, BertForMultipleChoice)
from transformer import (RobertaConfig, RobertaTokenizer, RobertaForMultipleChoice, RobertaForMCWithLS)

from transformer import AdamW, get_linear_schedule_with_warmup

from utils.dataset_utils import (Chn_Processor,
                                 convert_examples_to_features)

from utils.dataset_base import select_field_MC
...
CONVERTER = {
    "chn": (convert_examples_to_features, select_field_MC)
}

# TaskName_NetworkName
MODEL_CLASSES = {
    "chn_basic_bert": (BertConfig, BertForMultipleChoice, BertTokenizer),
    "chn_basic_roberta": (RobertaConfig, RobertaForMultipleChoice, RobertaTokenizer)
}
...
def load_and_cache_examples(args, task, tokenizer, do_mode="dev", output_mode="classification"):
    ...
    cased_type = "uncased" if args.do_lower_case else "cased"
    ...
    field_selector = CONVERTER[args.task_name.split("_")[0]][1]
    ### TODO: if change the task, may need to modify the TensorDataset contain
    all_input_ids = torch.tensor(field_selector(features, "input_ids"), dtype=torch.long)
    all_input_mask = torch.tensor(field_selector(features, "input_mask"), dtype=torch.long)
    all_segment_ids = torch.tensor(field_selector(features, "segment_ids"), dtype=torch.long)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch_id in train_iterator:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        results = evaluate(args, model, tokenizer, prefix=global_step, train_loss=loss.item)
        ...
        if args.do_eval and args.evaluate_epoch:
            ...
            output_dir = os.path.join(args.output_dir, 'checkpoint-{}'.format(global_step))
            logger.info("Saving model checkpoint to %s, and global steps is %d", output_dir, global_step)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            # Take care of distributed/parallel training
            model_to_save = model.module if hasattr(model, 'module') else model
...
def evaluate(args, model, tokenizer, prefix=None, have_test_label=True, train_loss=-1.0, do_mode='dev'):
    eval_task = args.task_name
    eval_output_dir = args.output_dir
    eval_dataset, eval_features = load_and_cache_examples(args, eval_task, tokenizer, do_mode=do_mode)
    ...
    ### Eval!
    logger.info(f"Logging file: [{args.log_file}]")
    logger.info("***** Running {} - {} *****".format(do_mode, prefix))
    ...
    detail_result_fp = "result.detail.{}.txt".format(do_mode)
    if do_mode == "dev":
        detail_result_fp = args.result_eval_file
    if do_mode == "test":
        detail_result_fp = args.result_test_file
    if do_mode == "trial":
        detail_result_fp = args.result_trial_file
    output_eval_file = os.path.join(eval_output_dir, "{}-{}".format(detail_result_fp, prefix))
    logger.info("Eval result write to: {}".format(output_eval_file))
    if have_test_label:
        ...
        with open(output_eval_file, "w") as writer:
            logger.info("***** Mode-{} results {} *****".format(do_mode, prefix))
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for i, (pred, prob) in enumerate(zip(pred_out, pred_prob)):
                qid = eval_features[i].example_id
                result_out = "\t".join(["{:.4f}".format(pi) for pi in prob])
...
def main():
    ...
    # tokenizer = tokenizer_class.from_pretrained (args.tokenizer_name_or_path)
    # tokenizer = tokenizer_class.from_pretrained ("google/roberta2roberta_L-24_cnn_daily_mail",
    #                                              do_lower_case=args.do_lower_case)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=global_step, have_test_label=True, do_mode='dev')
            result = dict(('{}_'.format('dev') + k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)

    # Trial Evaluation or unofficial Evaluation
    if args.do_trial and args.local_rank in [-1, 0]:
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
            checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
            logging.getLogger("pytorch_transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging ??
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=global_step, have_test_label=True, do_mode='trial')
            result = dict(('{}_'.format('trial') + k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
